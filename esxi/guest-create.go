...
func guestCREATE(c *Config, guest_name string, use_vmkfstools string, disk_store string,
	src_path string, src_path_vmx string, resource_pool_name string, strmemsize string, strnumvcpus string, strvirthwver string, guestos string,
	...
	if vmid != "" {
	...
	} else if strings.HasPrefix(src_path, "/vmfs/volumes/") {

		fmt.Println("Will use vmkfstools to clone VM...")
		...
		// Get SCSI type of source VM bootdisk
		remote_cmd := fmt.Sprintf("awk -F '=' '/scsi0.virtualDev/ {gsub(/^[[:space:]]+|[[:space:]]+$/, \"\", $2); gsub(/\\\"/, \"\", $2); print $2}' \"%s\"", src_path_vmx)
		stdout, _ = runRemoteSshCommand(esxiConnInfo, remote_cmd, "get SCSI type of source VM bootdisk.")
		scsi_bootdisk_type = stdout
		fmt.Println("SCSI Bootdisk Type:", scsi_bootdisk_type)
		//if strings.Contains(stdout, "") == true {
		//remote_cmd = fmt.Sprintf("mkdir \"%s\"", fullPATH)
		//stdout, err = runRemoteSshCommand(esxiConnInfo, remote_cmd, "get SCSI type of source VM.")
		if err != nil {
			log.Printf("[guestCREATE] Failed to get SCSI type of source VM bootdisk. src_path:%s\n", src_path)
			return "", fmt.Errorf("Failed to get SCSI type of source VM bootdisk. src_path:%s\n", src_path)
		}
		//}
		...
		vmx_contents =
			fmt.Sprintf("config.version = \\\"8\\\"\n") +
				...
				fmt.Sprintf("scsi0.virtualDev = \\\"%s\\\"\n", scsi_bootdisk_type) +
		...
		//  Clone disk (vmdk)
		remote_cmd = fmt.Sprintf("vmkfstools -d thin -i \"%s\" \"%s/%s.vmdk\"", src_path, fullPATH, guest_name)
		_, err = runRemoteSshCommand(esxiConnInfo, remote_cmd, "vmkfstools (clone disk)")
		if err != nil {
			...
			log.Printf("[guestCREATE] Failed to vmkfstools (clone disk):%s\n", err.Error())
			return "", fmt.Errorf("Failed to vmkfstools (clone disk):%s\n", err.Error())
		}
